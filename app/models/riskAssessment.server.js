...
async function validateWithOla(orderId, fullAddress) {
  try {
    ...
    console.log(`[Address API - OLA] Sending GET request for Order ${orderId}...`);

    const response = await fetch(url, {
      method: "GET",
      headers: { "X-Request-Id": orderId.toString() }
    });

    if (!response.ok) {
      console.error(`[Address API - OLA] HTTP Error ${response.status}`);
      return null; 
    }
    ...
    console.error("[Address API - OLA] Exception occurred:", error.message);
    return null; 
  }
}

// --- 2. FALLBACK API: MAPONOMY ---
async function validateWithMaponomy(orderId, fullAddress) {
  try {
    const cleanAddress = fullAddress.replace(/,/g, '');
    const apiPayload = {
      places: [
    ...
    console.log(`[Address API - MAPONOMY] Sending Payload for ${orderId}:`, JSON.stringify(apiPayload, null, 2));

    const response = await fetch("https://maponomy2.potterstech.com/api/v3/clients/places/geocode", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": process.env.MAPONOMY_API_KEY
      },
    ...
    if (!response.ok) {
      ...
      console.error(`[Address API - MAPONOMY] HTTP Error ${response.status} Details:`, errorText);
      return null;
    }

    const responseData = await response.json();
    console.log(`[Address API - MAPONOMY] Response received for order ${orderId}:`, JSON.stringify(responseData, null, 2));
    ...
    if (responseData.success && responseData.data) {
      ...
      if (responseData.data.outside_places && responseData.data.outside_places.length > 0) {
        console.log(`[Address API - MAPONOMY] Address found in 'outside_places'. Treating as a valid real-world address.`);
        return true;
      }

      // 3. If both arrays are empty, the address could not be found anywhere.
      console.log(`[Address API - MAPONOMY] Address not found in 'places' or 'outside_places'. Treating as invalid/fake.`);
      return false;
    }

    return null; 
  } catch (error) {
    console.error("[Address API - MAPONOMY] Exception occurred:", error.message);
    return null; 
  }
}

// --- 3. ORCHESTRATOR: MAIN EXPORTED FUNCTION ---
export async function checkAddressValidity(orderId, fullAddress) {
  if (!fullAddress || fullAddress.trim() === "") return null;

  // STEP 1: Try Primary API (Ola Maps)
  let validationResult = await validateWithOla(orderId, fullAddress);

  if (validationResult !== null) {
      console.log(`[Address API] Successfully validated using Ola Maps for Order ${orderId}`);
      return validationResult;
  }

  // STEP 2: Primary failed, initialize Fallback (Maponomy)
  console.warn(`[Address API] Ola Maps failed/inconclusive. Initiating fallback to Maponomy for Order ${orderId}...`);
  validationResult = await validateWithMaponomy(orderId, fullAddress);

  if (validationResult !== null) {
      console.log(`[Address API] Successfully validated using Maponomy for Order ${orderId}`);
      return validationResult;
  }

  // STEP 3: Both APIs failed
  console.error(`[Address API] BOTH validation providers failed for Order ${orderId}.`);
  return null; 
}
...
// 1. Get the risk percentage (Handling both possible variable names from Python)
  ...
  // 10. TRIGGER OMNICHANNEL NOTIFICATIONS & WHATSAPP FLOW
    try {
        ...
        try {
            // ONLY send standard confirmation if the address is perfect
            if (isAddressValid !== false && isAddressValid !== null && hasHouseNumber) {
                ...
                console.log(`[Order Confirmation] Skipped standard confirmation. Address requires verification.`);
            }
        } catch (orderConfirmError) {
            console.error("[Order Confirmation Error]:", orderConfirmError);
        }

        // --- TRIGGER MYOPERATOR WHATSAPP TEMPLATE ---
        if (customerPhone) {
            const appUrl = process.env.APP_URL || "https://zippyy.com";
            const dynamicEditUrl = `${appUrl}/edit-address/${secureToken}`;
            
            // Default setup for perfectly VALID addresses
            let waCampaignName = "address_verification"; 
            let dynamicTemplateParams = [
                secureToken,                        
                customerName || "Customer"          
            ];
            
            // If the address failed or is missing a house number, trigger the verification template
            if (isAddressValid === false || isAddressValid === null || !hasHouseNumber) {
            ...
            const whatsappApiKey = process.env.WHATSAPP_API_KEY || process.env.MYOPERATOR_API_KEY;
            await fetch("https://backend.api-wa.co/campaign/myoperator/api/v2", {
                ...
                body: JSON.stringify({
                    ...
                    destination: customerPhone.replace(/\D/g, ''),
                    userName: customerName || "Customer",
                    templateParams: dynamicTemplateParams,
            ...
            .then(async (res) => {
                if (!res.ok) {
                    ...
                    console.error(`[WhatsApp API Error] Failed to send ${waCampaignName}. HTTP ${res.status}:`, errBody);
                } else {
                    console.log(`[WhatsApp API] Successfully sent template '${waCampaignName}' to order ${orderGid}`);
                }
            })
