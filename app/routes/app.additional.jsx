...
export const loader = async ({ request }) => {
  ...
  const profilesData = await prisma.zippyy_buyer_profile.findMany({
    where: { shop },
    orderBy: { totalCheckoutAttempts: "desc" }, // Changed to sort by most active buyers first
  });
  ...
  const profiles = profilesData.map((p) => {
    ...
    // 🔥 FIX: Clean Up the Display Name (Hide the GID string)
    ...
    return {
      ...p,
      displayName, // The clean name we will show in the table
...
export default function Index() {
  ...
  const rowMarkup = filteredProfiles.map((profile, index) => {
    return (
      ...
      >
        ...
        {/* 🔥 FIX: Showing BOTH Total Checkouts and Valid Orders */}
        <IndexTable.Cell>
  ...
  return (
    <Page title="Buyer Profiling CRM" primaryAction={<Button onClick={handleRefresh} loading={isRefreshing}>Refresh Data</Button>}>
      <Layout>
        {/* HERO KPIs */}
        <Layout.Section>
