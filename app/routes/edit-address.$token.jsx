...
async function updateShopifyOrderAndCustomerAddress(shop, orderId, newAddress) {
  ...
  // 3. Update Customer's Default Profile Address 
  if (customerId) {
...
export async function loader({ params }) {
  ...
  const orConditions = [];
  if (displayOrder.customerPhone) orConditions.push({ customerPhone: displayOrder.customerPhone });
  if (displayOrder.customerEmail) orConditions.push({ customerEmail: displayOrder.customerEmail });
  if (displayOrder.customerId) orConditions.push({ customerId: displayOrder.customerId });
...
export async function action({ request, params }) {
  ...
  const inputAddress1 = formData.get("address1")?.toString() || "";
  const inputCity = formData.get("city")?.toString() || "";
  const inputProvince = formData.get("province")?.toString() || "";
  const inputZip = formData.get("zip")?.toString() || "";
  ...
  // Call your API logic
  const isAddressValid = await checkAddressValidity(order.shopifyOrderId, fullAddressString);

  // If the API explicitly returns false, OR if there's no house number, reject the submission.
  if (isAddressValid === false || !hasHouseNumber) {
    return Response.json({ 
      error: "We couldn't verify this address. Please ensure you include a valid House/Flat/Plot number and correct street details." 
    }, { status: 400 });
  }

  // --- IF PASSED, PROCEED WITH UPDATES ---
  const newAddress = {
  ...
  try {
    ...
    // B. Update local Database & INVALIDATE TOKEN
    await prisma.shopify_store_order.update({
      ...
      data: {
        ...
        addressVerified: true,
        addressEditToken: `WEB_EDITED_${token}` // This prevents future edits
      }
    });

    // C. ADD TAG
    await addTagToShopifyOrder(order.shop, order.shopifyOrderId, "Address: Edited");
    ...
    try {
      ...
      console.error("[Edit Address] Failed to send WhatsApp confirmation:", msgError);
    }
...
export default function EditAddress() {
  ...
  if (order.addressEditToken && (order.addressEditToken.startsWith("WEB_USED_") || order.addressEditToken.startsWith("WEB_EDITED_")) && !actionData?.success) {
    return (
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "400px", margin: "0 auto", fontFamily: "sans-serif" }}>
      ...
      {/* If the validation in the action fails, it renders this error box without resetting the form */}
      {actionData?.error && (
        <p style={{ color: "#b91c1c", padding: "12px", backgroundColor: "#fef2f2", border: "1px solid #f87171", borderRadius: "6px", fontSize: "14px", lineHeight: "1.4" }}>
          {actionData.error}
        </p>
      )}
      ...
      <Form method="post" style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
        <label>
          <span style={{ display: "block", marginBottom: "5px", fontSize: "14px", fontWeight: "bold" }}>Street Address (incl. House/Flat No.)</span>
        ...
        >
          {isSubmitting ? "Verifying..." : "Save & Confirm Order"}
        </button>
      </Form>
    </div>
  );
...
//         shippingZip: newAddress.zip,
//         addressVerified: true,
//         addressEditToken: `WEB_EDITED_${token}`
//       }
//     });
