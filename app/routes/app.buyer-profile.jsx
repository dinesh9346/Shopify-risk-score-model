import { useLoaderData, useNavigation, useSubmit, useSearchParams, useNavigate } from "react-router";
...
export default function Index() {
  ...
  const buildEvidenceUrl = (disputeId) => {
    const params = new URLSearchParams();
    params.set("disputeId", disputeId);
    return `/app/generate-evidence?${params.toString()}`;
  };
  ...
  const handleDownloadEvidence = (disputeId) => {
    if (!disputeId) return;
    navigate(buildEvidenceUrl(disputeId));
  };
  ...
  const getOrderBucket = (order) => {
    if (order.hasDispute || (order.disputes && order.disputes.length > 0)) {
      return "disputed";
    }
  ...
  const getShipmentBadge = (order) => {
    ...
    if (bucket === "disputed") return <Badge tone="critical">Disputed</Badge>; // ADDED BADGE FOR DISPUTED
  ...
  // NEW: Added the Disputed tab here so it displays in the UI!
    ...
    return (
      ...
      >
        <Layout>
          ...
          {/* ORDER & TRACKING HISTORY WITH DISPUTE BANNER */}
          <Layout.Section>
            <Card padding="0">
              <Box padding="400">
                  ...
                  <BlockStack gap="300">
                    {filteredOrderHistory.length > 0 ? (
                      filteredOrderHistory.map((order, idx) => {
                        ...
                        const activeDispute = order.disputes && order.disputes.length > 0 ? order.disputes[0] : null;

                        return (
                          <Card key={order.id || idx} roundedAbove="sm" background="bg-surface-secondary">
                            <BlockStack gap="300">
                              
                              {/* NEW: DISPUTE ACTION BANNER */}
                              {activeDispute && (
                                <Banner
                                  title={`Chargeback Filed: ${activeDispute.reason.toUpperCase().replace("_", " ")}`}
                                  tone={activeDispute.status === "won" ? "success" : "critical"}
                                  action={{
                                    content: 'Generate Evidence File',
                                    icon: ExportIcon,
                                    onAction: () => handleDownloadEvidence(activeDispute.shopifyDisputeId),
                                  }}
                                >
                                  <p>
                                    <strong>Status:</strong> {activeDispute.status.replace("_", " ")} | 
                                    <strong> Amount Contested:</strong> {activeDispute.amount} {activeDispute.currency}
                                  </p>
                                </Banner>
                              )}
...
//     const handleDownloadEvidence = (disputeId) => {
//       window.open(`/api/generate-evidence?disputeId=${disputeId}`, '_blank');
//       shopify.toast.show('Generating evidence file...');
//     };
